...
/*
Sintaxe:
SELECT count(NomeDaTabela.NomeDaColuna)
FROM NomeDaTabela;
Função:
Calcula a quantidade de vezes em que um determinado valor é inserido numa certa tabela
*/
...
SELECT count(dc.id) AS 'Número de Docentes'
FROM Docente dc;

SELECT count(di.id) AS 'Número de Disciplinas'
FROM Disciplina di;

SELECT count(t.id) AS 'Número de Turmas'
FROM Turma t;

SELECT count(e.id) AS 'Número de Estudantes'
FROM Estudante e;

SELECT count(r.id) AS 'Número de Responsáveis'
FROM Responsavel r;

SELECT min(dc.data_de_nascimento) AS 'Docente mais velho'
FROM Docente dc;

SELECT max(dc.data_de_nascimento) AS 'Docente mais novo'
FROM Docente dc;


SELECT DISTINCT concat(di.serie, "º ano ",di.nome) AS 'Séries / Disciplinas'
FROM Disciplina di
WHERE nome != 'Matemática' && nome != 'Geografia';

SELECT concat(dc.nome, " ",dc.sobrenome) AS 'Docente'
FROM Docente dc;
...
-- Algumas colunas podem conter valores duplicados. Para exibir valores diferentes ("distintos"), use a palavra-chave DISTINCT.
SELECT DISTINCT di.nome AS 'Disciplina'
FROM Disciplina di;

-- Cláusula WHERE permite filtrar registros em uma consulta.
SELECT * FROM Disciplina di
WHERE nome = 'Matemática' OR nome = 'Geografia';

SELECT * FROM Disciplina di
WHERE serie = 3 AND nome = 'Biologia';

SELECT id AS 'Id', nome AS 'Nome' 
FROM Docente dc
WHERE dc.sobrenome NOT LIKE 'de Souza Álvares';

SELECT id AS 'Id', nome AS 'Nome'
FROM Estudante e
WHERE data_de_nascimento
BETWEEN '1950-01-01' AND '1970-12-31';